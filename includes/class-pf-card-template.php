...
class PF_Card_Template {
	...
	private function extract_loop_body( $source ) {
		...
		// В исходном файле этот кусок начинается ВНУТРИ уже открытого <?php ... ?>
		// (сразу после the_post();) — своего <?php у него нет. Как отдельный
		// include-файл он должен сам открывать PHP-режим, иначе всё до первого
		// настоящего <?php внутри снипета (например, $rotation++; из примера
		// темы) выведется как обычный текст, а не выполнится.
		return '<?php ' . $snippet;
	}
