	...
	private function render_with_extracted_template( WP_Query $query, $cache_file ) {
		...
		try {
			while ( $query->have_posts() ) {
				...
				// $query — ВТОРИЧНЫЙ запрос (не главный запрос страницы), а хук
				// the_post(), на который WooCommerce вешает установку $product,
				// в таких запросах ненадёжен: сам факт срабатывания global
				// $post, $product выше не гарантирует, что WooCommerce успела
				// проставить туда актуальный объект — он может остаться null,
				// хотя на обычной (главной) загрузке страницы тот же код темы
				// получает $product корректно. Поэтому не полагаемся на хук,
				// а выставляем $product явно и детерминированно сами — так же,
				// как это рекомендует делать сама WooCommerce для вторичных
				// запросов.
